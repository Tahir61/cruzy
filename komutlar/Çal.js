...
module.exports.run = async (client, message, args) => {
  ...
  if (!user)
    return message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `⛔ Parasını çalmak istediğin kullanıcıyı etiketlemelisin!`
        )
    );
  ...
  if (author !== null && timeout - (Date.now() - author) > 0) {
    ...
    if (gard === "var")
      return message.channel.send(
        "Dostum bu adamın bir bodyguardı var hemen kaçmalısın"
      );
    if (author2 < 500) {
    ...
    let moneyEmbed2 = new Discord.MessageEmbed()
      ...
      .setDescription(
        `⛔ Faqir birisini soymaya çalıştın ve ${lostmoney} 💸 ceza yedin`
      );
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `✅ Soygun Başarılı, ${user}'ı soydun ve ${gotmoney} 💸 kazandın`
      )
      .setColor("GREEN");
