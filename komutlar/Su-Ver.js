...
exports.run = async (client, msg, args) => {
  ...
  let times = await db.fetch(`sutime_${msg.author.id}`);
  let day = 86400000 / 5;
  if (times !== null && day - (Date.now() - times) > 0) {
    ...
    msg.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `⏱ Kedine su vermek için ${time.hours ? time.hours + " saat" : ""} ${
  ...
  msg.channel.send(
    new Discord.MessageEmbed()
      .setTitle("Kedin artık susuz değil")
      .setColor("#91ffbb")
      .setDescription("Kedine su verdin")
  );

  let veri1 = [];

  veri1 = verilecek;
  db.set(`sutime_${msg.author.id}`, Date.now());
  ...
  if ((await db.fetch(`kedixp_${id}`)) > veri2) {
    msg.channel.send(
      new Discord.MessageEmbed()
        .setTitle("KEDİN ARTIK SUSUZ DEĞİL")
        .setDescription(
